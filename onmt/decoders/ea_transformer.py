...
subsequent EA_Transformer based architectures
"""
...
class EA_TransformerDecoderLayerBase(nn.Module):
    def __init__(
        self,
        d_model,
        heads,
        ...
        """
        super(EA_TransformerDecoderLayerBase, self).__init__()
...
class EA_TransformerDecoderLayer(EA_TransformerDecoderLayerBase):
    """EA_Transformer Decoder layer block in Pre-Norm style.
    Pre-Norm style is an improvement w.r.t. Original paper's Post-Norm style,
    providing better converge speed and performance. This is also the actual
    implementation in tensor2tensor and also avalable in fairseq.
    See https://tunz.kr/post/4 and :cite:`DeeperEA_Transformer`.
    ...
    def __init__(
        ...
        """
        Args:
            See EA_TransformerDecoderLayerBase
        """
        super(EA_TransformerDecoderLayer, self).__init__(
            d_model,
            heads,
            d_ff,
            dropout,
        ...
        self.ea_attn = MultiHeadedAttention(
            heads, d_model, dropout=attention_dropout
        )
    ...
    def update_dropout(self, dropout, attention_dropout):
        super(EA_TransformerDecoderLayer, self).update_dropout(
            dropout, attention_dropout
        )
    ...
    def _forward(
        ...
        """A naive forward pass for EA_Transformer decoder.
        ...
        output = self.feed_forward(self.drop(last)+mid)

        return output, attns


class EA_TransformerDecoderBase(DecoderBase):
    def __init__(self, d_model, copy_attn, embeddings, alignment_layer):
        super(EA_TransformerDecoderBase, self).__init__()

        self.embeddings = embeddings

        # Decoder State
        self.state = {}
        self.ea_emb = torch.nn.Embedding(num_embeddings=28, embedding_dim=512)
    ...
    def update_dropout(self, dropout, attention_dropout):
        ...
        for layer in self.EA_Transformer_layers:
            layer.update_dropout(dropout, attention_dropout)


class EA_TransformerDecoder(EA_TransformerDecoderBase):
    """The EA_Transformer decoder from "Attention is All You Need".
    ...
    def __init__(
        ...
        super(EA_TransformerDecoder, self).__init__(
            d_model, copy_attn, embeddings, alignment_layer
        )

        self.EA_Transformer_layers = nn.ModuleList(
            [
                EA_TransformerDecoderLayer(
                    d_model,
                    heads,
                    d_ff,
                    dropout,
    ...
    def forward(self, tgt, memory_bank=None, ea=None, step=None, **kwargs):
        ...
        ea = ea.transpose(0, 1)
        ea_memory_bank = self.ea_emb(ea).squeeze(0)
        ...
        for i, layer in enumerate(self.EA_Transformer_layers):
            layer_cache = (
...
class EA_TransformerLMDecoderLayer(EA_TransformerDecoderLayerBase):
    """EA_Transformer Decoder only layer block in GPT style.
...
class EA_TransformerLMDecoder(EA_TransformerDecoderBase):
    """The EA_Transformer decoder from GPT-2
    ...
    def __init__(
        ...
        super(EA_TransformerLMDecoder, self).__init__(
            d_model, copy_attn, embeddings, None
        )
        self.EA_Transformer_layers = nn.ModuleList(
            [
                EA_TransformerLMDecoderLayer(
                    d_model,
                    heads,
                    d_ff,
                    dropout,
    ...
    def init_state(self, src=None, memory_bank=None, enc_hidden=None):
        super(EA_TransformerLMDecoder, self).init_state(None, None, None)
    ...
    def forward(self, tgt, memory_bank=None, step=None, **kwargs):
        ...
        with_align = kwargs.pop("with_align", False)
        assert not with_align, "EA_TransformerLMDecoder does not support align"
